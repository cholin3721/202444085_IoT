// TestC.nc는 모듈 컴포넌트로, 센서 노드의 핵심 로직(센서 데이터 읽기, 무선 전송, LED 제어 등)을 구현
...
// 모듈 컴포넌트의 역할: TestC.nc는 센서 노드의 핵심 로직을 구현합니다. 부팅, 타이머, 센서 읽기, 데이터 전송, LED 제어를 모두 처리하며, 이벤트 중심으로 동작합니다.
// 작동 흐름:
// 부팅: 시스템 시작 시 Boot.booted에서 메시지 버퍼를 초기화하고 타이머를 시작.
// 타이머: 10.24초마다(TEST_PERIOD) MilliTimer.fired가 호출되어 무선 모듈을 켬.
// 무선 시작: RadioControl.start로 무선 모듈을 켜고, 완료 시 센서 읽기 시작.
// 센서 읽기: step 변수를 사용해 온도 → 습도 → 조도 → 배터리 순으로 읽음. 각 센서 읽기는 비동기적으로 readDone 이벤트로 처리.
// 데이터 전송: 모든 데이터 수집 후 sendTask에서 메시지를 패킹해 브로드캐스트.
// 무선 종료: 전송 후 LED를 끄고 무선 모듈을 종료.
// LED 사용:
// LED0: 무선 시작 시 토글, 오류 시 켜짐(주석 처리됨).
// LED1: 습도 오류 시 켜짐(주석 처리됨).
// LED2: 데이터 전송 시 토글.
// 오류 처리: 센서 읽기 실패 시 고유 오류 코드(0xFFFA, 0xFFFB, 0xFFFC)를 설정해 수신 측에서 오류를 인지 가능.
// 학습 포인트
// 이벤트 중심 프로그래밍: nesC는 비동기적입니다. readDone 같은 이벤트가 센서 데이터를 반환하므로 콜백 처리에 익숙해져야 합니다.
// 태스크 (task): post readTask()처럼 태스크는 즉시 실행되지 않고 스케줄링됩니다. 이는 TinyOS의 비선점형 멀티태스킹을 지원.
// 인터페이스 사용: uses로 선언된 인터페이스는 명령(call)과 이벤트(event)로 구성됩니다. 예: Temp.read는 명령, Temp.readDone은 이벤트.
// TinyOS 메시징: message_t와 AMSend는 TinyOS의 Active Message 시스템으로, 무선 통신을 추상화합니다.
